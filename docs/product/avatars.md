...
5. [LLM Avatar Creation Prompt Pipeline](#5-llm-avatar-creation-prompt-pipeline)
   ...
   - [D — Abbreviation Avatar](#d--abbreviation-avatar)
...
### D — Abbreviation Avatar

> **Design intent:** a lightweight code-generated fallback avatar — initials on a
> colored background — is produced in parallel so the UI always has something to
> display while the image model runs.

Generated by **Pillow** (no LLM call):

- **Inputs**: DEMO:NAME (for initials), PHENO:BG_COLOR (background), `#FFFFFF` foreground
- **Output**: `avatar_abbr.png` — square raster, initials centered on solid background
