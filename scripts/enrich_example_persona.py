...
Usage:
    python scripts/enrich_celebrity_persona.py [--dry-run] [--overwrite] [--gateway-url URL] [--workers N]
...
_APPEARANCE_SCHEMA = {
    ...
    "properties": {
        ...
        "eye_shape": {
            "type": "string",
            "enum": ["almond", "round", "hooded", "monolid", "upturned", "downturned", "cat-eye"],
        },
        "brows_style": {
            "type": "string",
            "description": "Short description of brow shape and fullness",
        },
        "nose_shape": {"type": "string", "description": "Short description of nose shape"},
        "chin_shape": {"type": "string", "description": "Short description of chin shape"},
        "cheeks_shape": {"type": "string", "description": "Cheekbone and jaw description"},
        "hair_style": {"type": "string", "description": "Hair length and style description"},
        "clothing": {
            "type": "object",
...
_APPEARANCE_SYSTEM = (
    "You are a portrait analysis assistant. Examine the image carefully and extract precise "
    "visual attributes. All hex color values must be in the format '#RRGGBB'. "
    "Respond ONLY with valid JSON matching the requested schema."
)
...
def _enrich_appearance(
    ...
    img_path = persona_path.parent / "original.jpg"
    if not img_path.exists():
        img_path = persona_path.parent / "original.png"
    if not img_path.exists():
        logger.warning("  No original image found in %s", persona_path.parent)
        return {}
...
def main() -> None:
    ...
    # ── Plan work: one future per network task, all submitted up-front ────────
    # Each persona may need up to two independent network calls:
    #   personal  → Wikipedia fetch + text_gen
    #   appearance → image_inspector
    # Flat submission avoids nested-executor deadlocks and maximises I/O overlap.
    ...
    with ThreadPoolExecutor(max_workers=args.workers) as pool:
        for path in persona_paths:
            ...
            need_personal = _missing_personal(personal, args.overwrite)
            need_appearance = _missing_appearance(appearance, args.overwrite)
        ...
        for item in work:
            ...
            try:
                _write_persona(item["path"], item["persona"], dry_run=args.dry_run)
                results["enriched"] += 1
            except Exception as exc:
