...
Used by:
    - scripts/learn/_benchmark.py
    - scripts/learn/_analyze.py
    - scripts/examples/normalize_example_personas.py
    - scripts/examples/audit_example_coverage.py
"""
...
def find_best_image(folder: Path) -> Path | None:
    """Return the path to the best available portrait in a persona folder.

    Search order:
      1. images/best.jpg   — canonical best after multi-image download
      2. images/001.jpg    — highest-scored image
      3. original.jpg      — legacy single-image fallback
      4. Any *.jpg / *.png directly in the folder (e.g. benchmark renders)
    """
    for candidate in (
        folder / "images" / "best.jpg",
        folder / "images" / "001.jpg",
        folder / "original.jpg",
    ):
        if candidate.exists():
            return candidate
    # Fallback: any image file directly in the folder
...
def load_image_metadata(folder: Path) -> dict | None:
    """Load images/metadata.json from a persona folder, or None if absent."""
